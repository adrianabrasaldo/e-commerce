<template>
  <div class="container">
    <div class="text-h5 text-weight-bold q-mb-md">Transaction List</div>
    <q-markup-table 
      separator="vertical" 
      bordered>
      <thead>
      ...
      <tbody>
        <tr v-for="transaction in transaction_list" :key="transaction.transaction_id">
          <td class="text-left">  {{ transaction.transaction_id }}</td>
          <td class="text-right"> {{ transaction.transaction_timestamp }}</td>
          <td class="text-right"> {{ transaction.transaction_total }}</td>
          <td class="text-right"> 
          ...
          <td class="text-right"> {{ transaction.transaction_change }}</td>
          <td class="text-right"> {{ transaction.transaction_method }}</td>          
          <td class="text-right">            
            <q-select 
              dense 
              dark
           transaction.transaction_change   square
              outlined 
          ...
          <td class="text-right">{{ transaction.user_fullname }}</td>
          <td class="text-right">
            <q-btn 
              round 
              icon="visibility" 
