...
products.post('/create_product', upload.array('files', 10), (req, res) => {
  const productData = {
      ...
      .then(product => {
        if (product.product_id) {
          for (let i = 0; i < req.files.length; i++) {
            image.push({product_img_name:req.files[i].filename, 
          ...
          res.json({ 
            status: product.product_name + ' product succesfully created!'
          })
        }
      })  
      .catch(err => {
...
products.get('/products', (req, res) => {
  sequelize.query(`SELECT products.product_id           AS 'id',
                        product_name                    AS 'name',
                        product_price                   AS price,
                        product_quantity                AS quantity,
                        IFNULL(rating.rating, 0)        AS 'rating',
...
products.post('/view_product', (req, res) => {
